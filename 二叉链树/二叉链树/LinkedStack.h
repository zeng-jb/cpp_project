...
struct LinkedNode                            //链表节点定义
{
	T data;                               //数据域
	LinkedNode<T>* link;                      //指针域
	LinkedNode(LinkedNode<T>* ptr = NULL) { link = ptr; }                             //仅初始化指针成员的构造函数
	LinkedNode(const T&item, LinkedNode<T>* ptr = NULL) {data=item, link = ptr; }     //初始化数据与指针成员的构造函数
};
...
{
	//将新元素x插入栈顶，即链头
	top = new LinkedNode<T>(x, top);             //创建新的含x的节点
